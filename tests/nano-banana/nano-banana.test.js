...
test('resolveNanoBananaApiKey prefers GOOGLE_API_KEY, then GEMINI_API_KEY, then OPENROUTER_API_KEY', () => {
  assert.deepEqual(resolveNanoBananaApiKey({ GOOGLE_API_KEY: 'google-key', GEMINI_API_KEY: 'gemini-key', OPENROUTER_API_KEY: 'or-key' }), {
    apiKey: 'google-key',
    source: 'GOOGLE_API_KEY',
    provider: 'google',
  });
  assert.deepEqual(resolveNanoBananaApiKey({ GEMINI_API_KEY: 'gemini-key', OPENROUTER_API_KEY: 'or-key' }), {
    apiKey: 'gemini-key',
    source: 'GEMINI_API_KEY',
    provider: 'google',
  });
  assert.deepEqual(resolveNanoBananaApiKey({ OPENROUTER_API_KEY: 'or-key' }), {
    apiKey: 'or-key',
...
test('buildOpenRouterApiRequest matches the OpenRouter chat completions shape', () => {
  assert.deepEqual(
    buildOpenRouterApiRequest({
      prompt: 'A slide hero image.',
      model: 'gemini-3-pro-image-preview',
      aspectRatio: '16:9',
      imageSize: '4K',
    }),
    {
      ...
      messages: [{ role: 'user', content: 'A slide hero image.' }],
...
test('buildOpenRouterApiRequest preserves already-prefixed model names', () => {
  const req = buildOpenRouterApiRequest({
    prompt: 'test',
    model: 'google/gemini-3.1-flash-image-preview',
    aspectRatio: '1:1',
    imageSize: '2K',
  });
  assert.equal(req.model, 'google/gemini-3.1-flash-image-preview');
});

test('extractOpenRouterImage extracts base64 image from choices', () => {
  const payload = extractOpenRouterImage({
    choices: [
      {
        message: {
          role: 'assistant',
          ...
          images: [
            {
              type: 'image_url',
              image_url: {
                url: `data:image/png;base64,${Buffer.from('or-image-bytes').toString('base64')}`,
              },
            },
          ],
        },
  ...
  assert.equal(Buffer.from(payload.bytes).toString(), 'or-image-bytes');
});
...
test('main uses OpenRouter endpoint when only OPENROUTER_API_KEY is set', async () => {
  const workspace = await mkdtemp(path.join(os.tmpdir(), 'nano-banana-or-'));
  const calls = [];
  const output = [];

  try {
    await main(
      ['--prompt', 'A mountain landscape', '--slides-dir', workspace],
      {
                      ...
                      images: [
                        {
                          type: 'image_url',
                          image_url: {
                            url: `data:image/png;base64,${Buffer.from('or-fake-bytes').toString('base64')}`,
                          },
                        },
                      ],
                    },
    ...
    assert.deepEqual(body.image_config, { aspect_ratio: '16:9', image_size: '4K' });

    const saved = await readFile(path.join(workspace, 'assets', 'nano-banana-a-mountain-landscape.png'));
    assert.equal(saved.toString(), 'or-fake-bytes');
  } finally {
    await rm(workspace, { recursive: true, force: true });
  }
});
