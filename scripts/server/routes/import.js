...
export function createImportRouter(ctx) {
  ...
  router.get("/api/import-file", async (_req, res) => {
    ...
    try {
      ...
      res
        .status(404)
        .json({ error: `Cannot read import file: ${err.message}` });
    }
  });
  ...
  router.post("/api/import-md", async (req, res) => {
    ...
    if (typeof mdContent === "string" && mdContent.trim()) {
      ...
      try {
        ...
        return res
          .status(400)
          .json({ error: `Cannot read file: ${err.message}` });
      }
    } else {
      return res
        .status(400)
        .json({ error: "Provide `content` or `filePath`." });
    }
    ...
    if (!ctx.generateMutex.tryAcquire())
      return res
        .status(409)
        .json({ error: "A generation is already in progress." });

    const selectedModel =
      typeof model === "string" && ALL_MODELS.includes(model.trim())
        ? model.trim()
        : CLAUDE_MODELS[0];
    const runId = randomRunId();

    broadcastSSE(ctx.sseClients, "planStarted", {
      runId,
      topic: "(MD Import)",
    });
    broadcastSSE(ctx.sseClients, "progress", {
      runId,
      phase: "plan",
      step: "Converting markdown to slide outline",
    });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      try {
        if (
          sourceType === "pdf" ||
          (req.is("application/pdf") && Buffer.isBuffer(req.body))
        ) {
          ...
          const result = await parseSource("upload.pdf", buffer, {
            vision: true,
            visionOutputDir: visionTmpDir,
          });
          applyPdfVision(result, "PDF");
        } else if (sourceType === "url" || sourceUrl) {
          const url =
            sourceUrl || (typeof req.body === "object" ? req.body?.url : "");
          ...
          const filePath =
            typeof req.body === "object" ? req.body?.filePath : "";
          if (!filePath)
            return res
              .status(400)
              .json({ error: "filePath required for pdf-path type." });
          const absPath = resolve(process.cwd(), filePath.trim());

          visionTmpDir = await mkdtemp(join(tmpdir(), "slides-grab-pdf-"));
          const result = await parseSource(absPath, null, {
            vision: true,
            visionOutputDir: visionTmpDir,
          });
          ...
          return res.status(400).json({
            error: "Specify sourceType (pdf, url, pdf-path) or upload a PDF.",
          });
        }
      } catch (err) {
        ...
        return res
          .status(400)
          .json({ error: `Source parsing failed: ${err.message}` });
      }
      ...
      if (!extractedText.trim()) {
        ...
        return res.status(400).json({
          error:
            "Extracted text is empty — the source may not contain readable content.",
        });
      }
      if (extractedText.length > 400_000)
        extractedText =
          extractedText.slice(0, 400_000) + "\n\n[… 원문 일부 생략됨]";
      ...
      const model =
        typeof modelRaw === "string" && ALL_MODELS.includes(modelRaw.trim())
          ? modelRaw.trim()
          : CLAUDE_MODELS[0];
      ...
      broadcastSSE(ctx.sseClients, "planStarted", {
        runId,
        topic: `(Doc Import: ${sourceLabel})`,
      });
      broadcastSSE(ctx.sseClients, "progress", {
        runId,
        phase: "plan",
        step: `Extracted text from ${sourceLabel}`,
      });
      res.json({
        runId,
        topic: `(Doc Import: ${sourceLabel})`,
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      if (files.length > MAX_IMPORT_FILES) {
        return res
          .status(400)
          .json({ error: `Too many files (max ${MAX_IMPORT_FILES}).` });
      }
      if (!ctx.generateMutex.tryAcquire()) {
        return res
          .status(409)
      ...
      if (estimatedSize > MAX_TOTAL_SIZE) {
        return res
          .status(400)
          .json({ error: "Total file size too large (max 10MB)." });
      }
      ...
      broadcastSSE(ctx.sseClients, "planStarted", {
        runId,
        topic: `(Multi-file Import: ${files.length} files)`,
      });
      broadcastSSE(ctx.sseClients, "progress", {
        runId,
        phase: "plan",
        step: `Parsing ${files.length} files: ${fileNames}`,
      });
      res.json({
        runId,
        topic: `(Multi-file Import: ${files.length} files)`,
...
async function parseFileEntry(entry, parseSource, visionTmpDir) {
  ...
  if (ext === ".pdf" && entry.base64) {
    ...
    const result = await parseSource(name, buffer, {
      vision: true,
      visionOutputDir: visionTmpDir,
    });
  ...
  (async () => {
    ...
    try {
      ...
      broadcastSSE(ctx.sseClients, "progress", {
        runId,
        phase: "plan",
        step: "Extracting content from files",
      });
      ...
      if (parsed.length === 0) {
        broadcastSSE(ctx.sseClients, "planFinished", {
          runId,
          success: false,
          message: "No readable content found in any of the uploaded files.",
          outline: null,
        });
        return;
      }
  ...
  (async () => {
    try {
      ...
      if (userPrompt) {
        ...
        promptLines.push(
          "위 사용자 지시사항을 반드시 반영하여 아웃라인을 작성하세요.",
        );
      }

      promptLines.push("", "다음을 수행하세요:", "");
      promptLines.push(
        `1. 주제에서 핵심 키워드 2~3개를 뽑아 영어 소문자 kebab-case 폴더명을 결정하세요.`,
      );
      promptLines.push(`   예: ${folderExample}`);
      promptLines.push("");
      promptLines.push(
        "2. 해당 폴더에 slide-outline.md를 생성하세요. (HTML 슬라이드는 생성하지 마세요)",
      );
      ...
      appendOutlinePrompt(promptLines, importPackId, {
        includePresenterNote: true,
        existingDeckNames,
      });
      ...
      if (success) {
        broadcastSSE(ctx.sseClients, "progress", {
          runId,
          phase: "plan",
          step: "Parsing generated outline",
        });
        try {
          ...
          if (bestDir) {
            ...
            const outlineContent = await readFile(
              join(decksRoot, bestDir, "slide-outline.md"),
              "utf-8",
            );
      ...
      broadcastSSE(ctx.sseClients, "planFinished", {
        ...
        message: success
          ? "Outline ready."
          : `Import failed (exit code ${result.code}).`,
        outline,
