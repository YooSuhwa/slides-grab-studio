...
export function getNanoBananaUsage() {
  return [
    ...
    '  GOOGLE_API_KEY    → Google Gemini API direct',
    '  GEMINI_API_KEY    → Google Gemini API direct',
    '  OPENROUTER_API_KEY → OpenRouter gateway (supports Gemini models)',
  ].join('\n');
}
...
export function resolveNanoBananaApiKey(env = process.env) {
  ...
  if (googleApiKey) {
    return { apiKey: googleApiKey, source: 'GOOGLE_API_KEY', provider: 'google' };
  }

  const geminiApiKey = typeof env?.GEMINI_API_KEY === 'string' ? env.GEMINI_API_KEY.trim() : '';
  if (geminiApiKey) {
    return { apiKey: geminiApiKey, source: 'GEMINI_API_KEY', provider: 'google' };
  }

  const openrouterApiKey = typeof env?.OPENROUTER_API_KEY === 'string' ? env.OPENROUTER_API_KEY.trim() : '';
  if (openrouterApiKey) {
    return { apiKey: openrouterApiKey, source: 'OPENROUTER_API_KEY', provider: 'openrouter' };
  }
...
export function getNanoBananaFallbackMessage(reason) {
  ...
  return `${summary} Ask the user to provide GOOGLE_API_KEY, GEMINI_API_KEY, or OPENROUTER_API_KEY, or fall back to web search + download the chosen image into ./assets/<file>.`;
}
...
export function sanitizeAssetName(value, maxLength = 80) {
  return value
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
...
export function buildOpenRouterApiRequest({ prompt, model, aspectRatio, imageSize }) {
  return {
...
export function extractOpenRouterImage(payload) {
  ...
  throw new Error('OpenRouter API response did not include an image payload.');
}
...
export async function generateNanoBananaImage({
  ...
  try {
    let endpoint, headers, body, extractFn;

    if (provider === 'openrouter') {
      endpoint = OPENROUTER_ENDPOINT;
      headers = { 'Content-Type': 'application/json', 'Authorization': `Bearer ${apiKey}` };
      body = JSON.stringify(buildOpenRouterApiRequest({ prompt, model, aspectRatio, imageSize }));
      extractFn = extractOpenRouterImage;
    } else {
      endpoint = buildNanoBananaEndpoint(model);
      headers = { 'Content-Type': 'application/json', 'x-goog-api-key': apiKey };
      body = JSON.stringify(buildNanoBananaApiRequest({ prompt, aspectRatio, imageSize }));
      extractFn = extractGeneratedImage;
    }
...
export async function runNanoBananaCli(argv = process.argv.slice(2), {
  ...
  const { apiKey, provider } = resolveNanoBananaApiKey(env);
  if (!apiKey) {
    throw new Error(getNanoBananaFallbackMessage('Nano Banana image generation requires GOOGLE_API_KEY, GEMINI_API_KEY, or OPENROUTER_API_KEY.'));
  }
